...
The most efficient way to extract job listings directly from WeWorkRemotely Jobs search results.

[View on Apify Store](#)

## 📖 Overview

The **WeWorkRemotely Jobs Scraper** is a powerful data extraction tool designed to aggregate job listings from the WeWorkRemotely Jobs search engine.
...
- **🌍 Multi-Source Aggregation** – Access listings from thousands of job boards via a single WeWorkRemotely search query.
...
|-----------|------|----------|-------------|---------|--------------|
| `countryName` | String | No | The country context for the WeWorkRemotely Search domain | "all" | "usa", "uk", "india", etc. |
...
| `companyName` | String | No | Filter listings to a specific employer | - | Any company name (e.g., "WeWorkRemotely", "Microsoft") |
    ...
    [
      {
        ...
        "date": "2025-03-25",
        "job_type": "Full-time",
        "URL": "https://www.WeWorkRemotely.com/search?..."
      }
    ]
...
- **posted_via:** The original platform where WeWorkRemotely found the job (e.g., Monster, Greenhouse, LinkedIn).
...
### Python
    ...
    run = client.actor("orgupdate/WeWorkRemotely-jobs-scraper").call(run_input=run_input)
...
### Node.js
    ...
    (async () => {
        // Run the Actor and wait for it to finish
        const run = await client.actor('orgupdate/WeWorkRemotely-jobs-scraper').call(input);
...
- **WeWorkRemotely Sheets:** Automatically save new job listings into a spreadsheet for analysis.
